...
def main():
	...
	for i in imgs:
		...
		# cv2.imshow("canny",cv2.Canny(cv2.cvtColor(warped,cv2.COLOR_BGR2GRAY),150,200))
		...
		# cv2.imshow("Lane Left",world_lane_left)
		# cv2.imshow("Lane Roght",world_lane_right)

		# plt.figure("Lane Left")
		# plt.imshow(world_lane_left)
		# plt.figure("Lane Right")
		# plt.imshow(world_lane_right)
		# plt.show()
		# sys.exit(0)
		...
		for i in np.arange(left[0,0],right[0,0]):
			top.append([i,left[0,1]])
		top = np.array(top)
		# print("Top",top)

		bottom = []
		for i in np.arange(left[-1,0],right[-1,0]):
			bottom.append([i,left[-1,1]])
